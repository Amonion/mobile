    ...
    <>
      {user?.username === userForm.username ? (
        <View className="bg-primary z-40 dark:bg-dark-primary flex-row justify-around pt-2 pb-1 border-b border-border dark:border-dark-border">
          <TouchableOpacity
            onPress={() => {
              if (pathname !== `/home/profile/my-profile`) {
          ...
          >
            <Text
              className={`${
                pathname === `/home/profile/my-profile`
                  ? 'text-custom'
          ...
          <TouchableOpacity
            onPress={() => {
              if (pathname !== `/home/profile/my-profile/comments`) {
                router.push(`/home/profile/my-profile/comments`)
              }
            }}
            className="items-center"
          >
            <Text
              className={`${
                pathname === `/home/profile/my-profile/comments`
                  ? 'text-custom'
                  : 'text-primary dark:text-dark-primary'
              } text-lg`}
            >
          ...
          <TouchableOpacity
            onPress={() => {
              if (pathname !== `/home/profile/my-profile/media`) {
                router.push(`/home/profile/my-profile/media`)
              }
            }}
            className="items-center"
          >
            <Text
              className={`${
                pathname === `/home/profile/my-profile/media`
                  ? 'text-custom'
                  : 'text-primary dark:text-dark-primary'
              } text-lg`}
            >
