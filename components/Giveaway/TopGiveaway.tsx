...
import { formatMoney, formatRelativeDate } from '@/lib/helpers'
import WeekendStore, { Weekend } from '@/store/exam/Weekend'

const TopGiveaways: React.FC = () => {
  const { giveaways } = WeekendStore()
  const { getComments } = CommentStore()
  const [topGiveaways, setTopGiveaways] = useState<Weekend[]>([])
  ...
  useEffect(() => {
    ...
    const todaysGiveaways = giveaways
      .filter((item) => {
    ...
    setTopGiveaways(todaysGiveaways)
  }, [giveaways])
